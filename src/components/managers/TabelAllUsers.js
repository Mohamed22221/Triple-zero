...
import { getManagers, handleListView } from '../../store/ManagersSlice';
...
const TableAllUsers = ({ HandelShowCustomer }) => {
    ...
    const UserDataSelector = useSelector(state => state.managers)
    ...
    useEffect(() => {
        if (statusBlackList) {
            const BlackList = UserDataSelector.managers.filter(statusItem => statusItem.status == 0)
            setUserData(BlackList)
            setSortValue('')
        } else {
            setUserData(UserDataSelector.managers)
            setSortValue('')
        }
    }, [UserDataSelector.managers, statusBlackList])


    useEffect(() => {
        dispatch(getManagers(1))
    }, [dispatch])

    const handlePageClick = (data) => {
        console.log('handlePageClick', data.selected);
        dispatch(getManagers(data.selected + 1))
    }
        ...
        <div className="main-table">
            ...
            <div className='style-main-sort'>
                ...
                <form className='form-search'>
                    <input type="search" placeholder='أبحث عن اسم المشرف'
